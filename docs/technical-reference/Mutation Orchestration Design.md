...
## Audience
This document aims to assist developers who need or want to understand how Stryker.Net parses and mutates C# source code. The current design is the result of several years of experience and maturation and may be intimidating at first.
...
Warning: sometimes modifying syntax nodes may be useless. 
...
Stryker strives to find a good balance between performance, concision and ease of maintenance. Having this goal in mind and having gone through design revisions, a few key principles emerged as important:
...
3. [`MutationContext`]: In charge of keeping track of the current state of the mutation process. It stores mutations (via [`MutationStore`]) until they are injected in the mutated syntax tree; it also tracks which mutators have been disabled via [Stryker comments] and other details.
4. [`CSharpNodeOrchestrator`]: the main entry point. While it was doing all the work in earlier version of Stryker, it is now responsible for initiating the needed orchestrators, mutators and mutant placer and keeping track of created mutations.
5. [`MutantPlacer]: serves as a single entry point to several helper functions, including the one for injecting mutations. Also in charge of providing the needed information for rollback logic.

While understanding how everything works together is non trivial, each class is focused on a clear responsibility, making it easier to fix any problem or adjusting/extending existing behavior. As you can see, this design mostly leverage 2 traditional patterns: strategy (mutators, orchestrator) and state (MutationContext and MutationStore).
It is now time to dive deeper into each of those classes.

## Mutators
As said previously, a mutator class is responsible for generating zero or more mutations of provided syntax nodes. Syntax nodes will be submitted to the all active mutators for consideration, so mutators need to be efficient, at least on the filtering front (can they mutate this node or not).

I mentioned earlier that mutator must implement the `IMutator` interface, as it is required for the mutation mechanism, but Stryker’s mutators actually inherit from `MutatorBase<T>`. Then mutators must:
...
3. Implement `IEnumerable<Mutation> ApplyMutation(T node, SemanticModel model)` which must returns a collection of mutations for the provide syntax node _node_.

You can look at [`StringEmptyMutator`] for an example of a mutator of average complexity.

## Node Orchestrators
These classes bear most of the complexity. First and foremost, they prescribe the  mutation workflow, that is in which order perform the important states: 

1. generate the mutation(s) (if any), 
2. mutate the child nodes (that is, perform this workflow for every child node)
3. inject these mutations with the adequate control structure (`if` or ternary operator).
...
- creating a modified version of a syntax node **results in having an orphaned syntax node**, I.e. outside any syntax tree (source file). One must reinfect it via a reverse recursion, that is creating a new version of the parent node with the modified node and so on up to the root
- it keeps a (mostly) natural read order for mutation : mutations appear in their id number.
They provide an implementation for the major syntax structure (expressions, statements, blocks) but also implement specific strategies for the subtler syntax constructs.
As a reminder, they are all stateless. These classes must not store anything while walking the syntax tree. They should not assume this walking happens in any specific order. And they are likely to be used recursively, depending on the currently parsed code.
We will describe the base class right now, all orchestrators will be described at the end of this document as an attempt to keep it readable.

###NodeSpecificOrchestrator<T, TU>
This is the base class used by all other orchestrators. It inherits from `NoOrchestratorBase` which deals with Stryker comments and is not described in this document.
It implements the standardized node mutation workflow. This workflow’s steps are implemented via virtual method so that specific orchestrators can provide the adequate implementation

- `Mutate(...)` this is the main method, which specify the steps order:

	1.  Invoke `PrepareContext` to get a `MutationContext` instance in the adequate state.
	2.  Invoke `GenerateMutationForNode` to get the list of possible mutations (if any) for the current node.
	3.  Invoke `StoreMutations` so that generated mutations are queued ‡for later injection
	4.  Invoke `OrchestrateChildrenMutation` to get the mutated children and prepare a mutated version of this node
	5.  Invoke `InjectMutations` to insert pending mutations, if the current node can support his.
	6.  Finally, invoke `RestoreContext` to get the adequate context when leaving this node. 
...
- `OrchestrateChildrenMutations(...)`: should build a mutated version of the node integrating all child nodes’ mutations. Default implementation does that. Child classes should override this method when they need to control how child nodes are mutated and/or adjust the context before or during children mutation.
...
Below is the list of its method, split between the ones that are useful for an orchestrator and the ones that are used by Stryker’s main logic (and are not needed when implementing an orchestrator
### Context management
These methods are used in the `PrepareContext(...)`/`RestoreContext(...)` methods.
Orchestrator generally must specify the proper syntax level (note that a parent class may have done it). Useful methods. for an orchestrator
...
- `Leave()`: returns a `MutationContext` restored when leaving a  `SyntaxNode` of significant hierarchical level (member, block, statement, expression, member access). When leaving a context level, any remaining mutations is transferred to the enclosing level (e.g. from expression to statement) so they can be injected. Any mutation that fail to be injected is flagged as compilation error and logged accordingly.
Used by main logic:
...
### Base classes
These classes are responsible for the general structure of the orchestration phase and/or are used as parent classes to other, more specific, orchestrators.
#### CsharpMutantOrchestrator
Note an orchestrator per se. This is the starting point for mutating C# source file. It aggregates the configured mutators and stores mutations are they are generated. It inherits from `BaseMutantOrchestrator<T,TU>`which implements the base logic needed by the overall workflow.
Note that there is an `FsharpMutantOrchestrator` class for a future support of F#.
- `MutantPlacer Placer` : this property stores the `MutantPlacer` instance that should be use for code injection
...
- `SwitchToThisBodies(...)`: Must replace the method body with the provided body (either block or expression). This method is used to perform the expression to block body conversion and for roll back as well
Orchestrator’s overrides:
- `InjectMutations(...)`: if the body is in expression form and there are mutations that have not been injected (because they require statement or block level control), converts the body to block form and inject those mutations. If the body is in block form, injects out parameters initialization to default and add an ending `return`  statement if it looks helpful.
Finally, this class implements an injection engine:
- `ConvertToBlockBody(...)` converts the method from expression form to block body form. Does nothing if the method as no expression body.
