...
### A word of warning
One of the consequence is that one must extensively tries syntax constructs to ensure all use cases are covered.
...
Another problem is worth mentioning: Roslyn documentation is lacking, as most of the doc is automatically generated from the signatures without any further comments or explanation. Last but not least, there is now way to know what king of syntax construct each class represents (except guessing).
...
Syntax nodes can be modified via a set of `WithXXX` methods which allows to change one of the node’s properties at a time. These calls can be chained via a builder pattern: `node.WithXXX(...).WithY(...)...'`.
...
While understanding how everything works together is non trivial, each class is focused on a clear responsibility, making it easier to fix any problem or adjusting/extending existing behavior. This design mostly leverage 2 traditional patterns: **strategy** (mutators, orchestrator) and **state** (MutationContext and MutationStore).
It is now time to dive deeper into each of those classes.

## Mutators
As noted previously, a mutator class is responsible for generating zero or more mutations of provided syntax nodes. Syntax nodes will be submitted to the **all active mutators for consideration**, so mutators need to be efficient, at least on the filtering front (can they mutate this node or not?).
...
This is the base class used by all other orchestrators. It inherits from `NoOrchestratorBase` which deals with Stryker comments and is not described in this document fro brevity.
...
It also implements injection methods that generates mutated version of syntax nodes (expression, statement or block) enclosed in the appropriate mutation switching construct: ternary operator(s) for expressions and if statement(s) for statements and blocks.
...
-  `PlaceStatementControlledMutations(…)`: build an `if` statement with the provided mutation as the `true` condition and the provided statement/block has the `false` condition. The `if` statement will be chained if there are more than one mutation.
...
Not an orchestrator per se. This is the starting point for mutating C# source file. It aggregates the configured mutators and stores mutations are they are generated. It inherits from `BaseMutantOrchestrator<T,TU>`which implements the base logic needed by the overall workflow.
...
## Conclusion
Congratulations for reading this 5K words document to its end. I hope you found useful, and most importantly, use it when working on this parts of the project.
