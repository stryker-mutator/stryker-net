...
public sealed class VsTestRunnerPool : ITestRunner
{
    ...
    private IEnumerable<ICoverageRunResult> CaptureCoverageInOneGo(IProjectAndTests project) => ConvertCoverageResult(RunThis(runner => runner.RunCoverageSession(TestGuidsList.EveryTest(), project.GetTestAssemblies(), project.HelperNamespace, project.IsFullFramework).TestResults), false);
        ...
        CoverageConfidence defaultConfidence, out CoverageRunResult coverageRunResult)
    {
        ...
        if (!Context.VsTests.ContainsKey(testCaseId))
        {
            ...
            var testCase = new VsTestCase(testResult.TestCase);
            Context.VsTests.Add(testCaseId, new VsTestDescription(testCase));
            unexpected = true;
        }
