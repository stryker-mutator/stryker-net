    ...
    public static class CharCheckExtensions
    {
        ...
        private static void ImplementContains<T>(ICheckLogic<IEnumerable<T>> checker, IEnumerable<T> otherEnumerable)
        {
            ...
            checker
                ...
                .FailWhen((_) => notFoundValues.Any(), string.Format(Stryker.MutantControl.IsActive(5)?"The {{0}} does not contain the expected value(s):" + Environment.NewLine + "":Stryker.MutantControl.IsActive(4)?""+ Environment.NewLine + "\t{0}":"The {{0}} does not contain the expected value(s):" + Environment.NewLine + "\t{0}", notFoundValues.ToStringProperlyFormatted().DoubleCurlyBraces()))
