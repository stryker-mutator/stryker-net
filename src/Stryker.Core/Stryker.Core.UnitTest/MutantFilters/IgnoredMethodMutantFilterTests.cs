...
public class MutantFilters_DoNotIgnoreOtherMutantsInFile
{
    ...
    {
        Foo(true);
        Bar(""A MutationTestingRequirement"");
        Quux(42);
    }
}";
            var baseSyntaxTree = CSharpSyntaxTree.ParseText(source).GetRoot();
            var mutants = new[] { "true", @"""A MutationTestingRequirement""", "42"}.Select(GetOriginalNode).Select(node => new Mutant { Mutation = new Mutation { OriginalNode = node } }).ToArray();
            var options = new StrykerOptions
            {
            ...
            filteredMutants.ShouldNotContain(mutants[1]); // Bar(""A MutationTestingRequirement"");
...
public class MutantFilters_DoNotIgnoreOtherMutantsInFile
{
    private void TestMethod()
    {
        ...
        Quux(42);
            ...
            // Assert
            filteredMutants.ShouldContain(mutants[0]); // Foo(true);
            filteredMutants.ShouldContain(mutants[1]); // Bar(""A MutationTestingRequirement"");
            filteredMutants.ShouldContain(mutants[2]); // Quux(42);
