...
namespace Stryker.Core.UnitTest.Initialisation
{
    public class ProjectOrchestratorTests : TestBase
    {
        ...
        [Fact]
        public void ShouldSupportFSharp()
        {
            // when a solutionPath is given and it's inside the current directory (basePath)
            var options = new StrykerOptions
            {
            ...
            var csprojPathName = _fileSystem.Path.Combine(_projectPath, "sourceproject.fsproj");
            var testCsprojPathName = _fileSystem.Path.Combine(_projectPath, "testproject.fsproj");
        ...
        private ProjectOrchestrator BuildProjectOrchestrator(Dictionary<string, IProjectAnalyzer> analyzerResults, out Mock<ITestRunner> mockRunner)
        {
            ...
            {
                buildalyzerAnalyzerManagerMock.Setup(x => x.GetProject(analyzerResult.Key)).Returns(analyzerResult.Value);
            }
        ...
        private Mock<IProjectAnalyzer> SourceProjectAnalyzerMock(string csprojPathName, string[] sourceFiles)
        {
            var properties = new Dictionary<string, string>
                { { "IsTestProject", "False" }, { "ProjectTypeGuids", "not testproject" }, { "Language", "C#" } };

            return BuildProjectAnalyzerMock(csprojPathName, sourceFiles, properties, new List<string>());
        }
