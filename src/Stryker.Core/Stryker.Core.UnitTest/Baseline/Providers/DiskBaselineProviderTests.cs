...
namespace Stryker.Core.UnitTest.Baseline.Providers
{
    public class DiskBaselineProviderTests : TestBase
    {
        ...
        public async Task ShouldWriteToDiskAsync()
        {
            ...
            // Act
            await sut.Save(JsonReport.Build(options, JsonReportTestHelper.CreateProjectWith().ToReadOnlyInputComponent(), It.IsAny<TestProjectsInfo>()), "baseline/version");
        ...
        public async Task ShouldLoadReportFromDiskAsync()
        {
            ...
            var report = JsonReport.Build(options, JsonReportTestHelper.CreateProjectWith().ToReadOnlyInputComponent(), It.IsAny<TestProjectsInfo>());
