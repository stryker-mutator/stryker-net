...
class Program
{
    void Method()
    {
        ...
        [Fact]
        public void ShouldNotAddDefaultReturnIfReturnExpressionIsEmpty()
        {
            var block = @"{
                var x = 42;
                return;
            ...
            var statements = GetMutations(block);
            statements.ShouldHaveSingleItem().ReplacementNode.ShouldBeOfType<BlockSyntax>().Statements.ShouldBeEmpty();
        }
