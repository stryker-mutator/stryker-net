...
public class MutationTestProcess : IMutationTestProcess
{
    ...
    public async Task<StrykerRunResult> TestAsync(IEnumerable<IMutant> mutantsToTest)
    {
        ...
        // Reload test runner to pick up the newly injected mutated assemblies.
        // This is critical for runners with process reuse (like MTP) that cache assemblies.
        if (Input.TestRunner is not null)
        {
            await Input.TestRunner.ReloadMutatedAssemblyAsync().ConfigureAwait(false);
        }
