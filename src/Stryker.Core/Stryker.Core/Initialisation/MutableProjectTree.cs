...
internal class MutableProjectTree(ProjectSimulatedBuildWrapper project, ILogger logger)
{
    ...
    public void LogAllAnalysisSummaries(bool optionsDiagMode)
    {
        ...
        if (!Targets.Any())
        {
            if (!project.AnalyzerLastResults.Any())
            {
                logger.LogInformation("  simulated build failed early. No details are available.");
            }
            else
            {
                logger.LogInformation("  no test project seems to reference this project. If this is a test project, ensure it has the property: <IsTestProject>true</IsTestProject> in its project file.");
            }
