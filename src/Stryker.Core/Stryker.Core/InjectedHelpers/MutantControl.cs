namespace Stryker
{
    public static class MutantControl
    {
        ...
        private static bool TryReadMutantFromFile(out int mutantId)
        {
            ...
            try
            {
                // Always read the file content on each call.
                // File timestamp-based caching is unreliable when mutations are written rapidly
                // (filesystem timestamp granularity may be 1 second or more).
                // For MTP runner with process reuse, we must detect every content change.
