...
{
    public record CSharpCompilingProcessResult(CSharpCompilation Compilation, EmitResult EmitResult, IEnumerable<int> RollbackedIds);
    ...
    public class CsharpCompilingProcess : ICSharpCompilingProcess
    {
        ...
        public CSharpCompilingProcessResult Compile(IEnumerable<SyntaxTree> syntaxTrees, Stream ilStream, Stream symbolStream)
        {
            ...
            {
                return new (Compilation, emitResult, RollbackedIds);
            }
            // compiling failed
