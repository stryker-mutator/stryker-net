...
internal sealed class SingleMicrosoftTestPlatformRunner : IDisposable
{
    ...
    /// <summary>
    /// Restarts all assembly test servers to load newly injected mutated assemblies.
    /// </summary>
    public async Task RestartServersAsync()
    {
        List<AssemblyTestServer> servers;
        lock (_serverLock)
        {
            servers = _assemblyServers.Values.ToList();
        }

        foreach (var server in servers)
        {
            _logger.LogDebug("{RunnerId}: Restarting server for assembly reload", RunnerId);
        ...
        TimeSpan? timeout = null)
    {
        ...
        try
        {
            ...
            // MTP protocol defines multiple failure states: 'failed', 'error', 'timed-out'
            // See: https://github.com/microsoft/testfx/blob/main/docs/mstest-runner-protocol/001-protocol-intro.md
            var failedTests = finishedTests.Where(x => x.Node.ExecutionState is "failed" or "error" or "timed-out").Select(x => x.Node.Uid).ToList();

            // Debug: Log all unique execution states seen
            var uniqueStates = finishedTests.Select(x => x.Node.ExecutionState).Distinct().ToList();
            _logger.LogDebug("{RunnerId}: Test execution states: [{States}], Failed count: {FailedCount}",
                RunnerId, string.Join(", ", uniqueStates), failedTests.Count);
            ...
            var errorMessagesStr = string.Join(Environment.NewLine,
                finishedTests.Where(x => x.Node.ExecutionState is "failed" or "error" or "timed-out")
                    .Select(x => $"{x.Node.DisplayName}{Environment.NewLine}{Environment.NewLine}Test {x.Node.ExecutionState}"));
