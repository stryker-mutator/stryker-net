...
string testAssemblyPath = Path.GetFullPath("C:\\Dev\\Repos\\stryker-net\\MTP Demo\\MsTestRunnerDemo\\TUnitTestProject\\bin\\Debug\\net8.0\\TUnitTestProject.dll");
...
try
{
    ...
    await RunTestsAsync(CancellationToken.None, testUidFilter: node => true);

    // Or run specific tests by filtering on discovered test UIDs
    //Example:
    //await RunTestsAsync(cancellationToken, testUidFilter: uid => uid.Contains("BasicTests"));
}
catch (Exception ex)
{
...
async Task CheckNetworkConnectivity()
{
...
async Task RunTestsAsync(CancellationToken cancellationToken, Func<MsTestRunnerDemo.Models.TestNode, bool>? testUidFilter = null)
{
    ...
    Console.WriteLine($"[RPC Test] Starting test host process in: {testAssemblyPath}");


    Console.WriteLine($"[RPC Test] Command: dotnet run -f net8.0 --server --client-port {port}");

    var cliProcess = Cli.Wrap("dotnet")
        .WithWorkingDirectory(Path.GetDirectoryName(testAssemblyPath))
        .WithArguments([
            testAssemblyPath,
            "--server",
            "--client-port",
    ...
    MsTestRunnerDemo.Models.TestNode[]? testNodes = null;
